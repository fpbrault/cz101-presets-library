...
pub fn bundle_auv3(
    ...
    fs::create_dir_all(&app_macos_dir)
        .map_err(|e| format!("Failed to create app MacOS dir: {}", e))?;
    fs::create_dir_all(&app_resources_dir)
        .map_err(|e| format!("Failed to create app Resources dir: {}", e))?;
    fs::create_dir_all(&plugins_dir).map_err(|e| format!("Failed to create PlugIns dir: {}", e))?;
    fs::create_dir_all(&frameworks_dir)
        .map_err(|e| format!("Failed to create Frameworks dir: {}", e))?;
    ...
    fs::create_dir_all(&appex_macos_dir)
        .map_err(|e| format!("Failed to create appex MacOS dir: {}", e))?;
    fs::create_dir_all(&appex_resources_dir)
        .map_err(|e| format!("Failed to create appex Resources dir: {}", e))?;
...
"#,
    ...
    let arches = match arch {
        ...
        Arch::Native => vec![if cfg!(target_arch = "aarch64") {
            "arm64"
        } else {
            "x86_64"
        }],
    ...
    crate::verbose!(
        verbose,
        "    Building host app executable ({})...",
        arch_str
    );

    let stub_main_path = workspace_root.join("packages/xtask/au-support/objc/stub_main.c");
...
fn generate_appex_stub_source(plugin_name: &str) -> String {
    format!(
        r#"// Auto-generated appex stub for {plugin_name}
...
fn install_auv3(bundle_dir: &Path, bundle_name: &str, verbose: bool) -> Result<(), String> {
    ...
    let executable_name = bundle_name.trim_end_matches(".app");
    let killall_app = Command::new("killall").arg(executable_name).output();
