...
fn generate_objc_class_names() {
  use std::{fs, path::PathBuf};

  let out_dir = PathBuf::from(std::env::var("OUT_DIR").expect("OUT_DIR must be set"));
  let objc_prefix = std::env::var("WRY_OBJC_CLASS_PREFIX").unwrap_or_default();
  let class_names = [
    (
      "DOCUMENT_TITLE_CHANGED_OBSERVER_CLASS_NAME",
      "DocumentTitleChangedObserver",
    ),
    ("WRY_DOWNLOAD_DELEGATE_CLASS_NAME", "WryDownloadDelegate"),
    (
      "WRY_NAVIGATION_DELEGATE_CLASS_NAME",
      "WryNavigationDelegate",
    ),
    ("WRY_WEB_VIEW_CLASS_NAME", "WryWebView"),
    ("WRY_WEB_VIEW_DELEGATE_CLASS_NAME", "WryWebViewDelegate"),
    ("WRY_WEB_VIEW_PARENT_CLASS_NAME", "WryWebViewParent"),
    (
      "WRY_WEB_VIEW_UI_DELEGATE_CLASS_NAME",
      "WryWebViewUIDelegate",
    ),
  ];

  let mut generated = String::from("// This file is auto-generated by build.rs.\n");
  for (const_name, class_name) in class_names {
    generated.push_str(&format!(
      "pub const {const_name}: &str = {value:?};\n",
      value = format!("{objc_prefix}{class_name}")
    ));
  }

  fs::write(out_dir.join("wry_objc_class_names.rs"), generated)
    .expect("failed to write generated Objective-C class names");
}
