...
describe('DuplicateReviewModal (browser)', () => {
  ...
  it('selects all except the suggested keep when button clicked', async () => {
    ...
    const groups = [makeDuplicateGroup('g1', ['Preset A', 'Preset B', 'Preset C'])]
    ...
    await user.click(screen.getByRole('button', { name: /select all except/i }))
  ...
  it('clears selection when Clear Selection button is clicked', async () => {
    ...
    expect(screen.getByRole('button', { name: /delete selected \(0\)/i })).toBeTruthy()
  })
