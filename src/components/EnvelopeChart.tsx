...
const EnvelopeChart: React.FC<EnvelopeChartProps> = memo(({ steps, endStep, label, color = '#a78bfa' }) => {
  ...
  activeSteps.forEach((step, i) => {
    ...
    const transitionX = Math.min(stepEndX, stepStartX + STEP_CELL_W * durationRatio)
  ...
  return (
    <div className="flex flex-col gap-0.5 min-w-0">
      <span className="text-xs text-base-content/50 font-mono tracking-wider">{label}</span>
      <svg
        width="100%"
        height={H}
        viewBox={`0 0 ${W} ${H}`}
      ...
      <div className="grid grid-cols-8 gap-1 mt-1">
        {Array.from({ length: 8 }).map((_, i) => {
          ...
          return (
            ...
            >
              <div className="text-[8px] leading-tight font-mono">{i + 1}</div>
              <div className="text-[8px] leading-tight font-mono">{inactive ? '--' : `R${step.rate}`}</div>
              <div className="text-[8px] leading-tight font-mono">{inactive ? '--' : `L${step.level}`}</div>
            </div>
          )
        })}
      </div>
