...
export async function addFactoryPresetsToLibrary(): Promise<number> {
	...
	await presetDatabase.syncPresets([
		...existingPresets,
		...missingFactoryPresets,
	]);
	return missingFactoryPresets.length;
}

export async function ensureFactoryPresetsOnFirstUse(): Promise<
...
export async function cloudRestorePresets(): Promise<number | null> {
	...
	const restoredUserPresets = presets.filter(
		(preset) => !isFactoryPreset(preset),
	);
...
export async function retrieveInternalBackupFromSynth(
	...
	for (let i = 0; i < 16; i++) {
		...
		results.push(
			mapToSetlistEntry(slot, programByte, sysexData, presetMatchIndex),
		);
		onProgress?.(slot, 16);
		await delay(60);
	}
...
export async function backupPresets(portName: string): Promise<void> {
	await mkdir(`${BACKUP_FOLDER}/user`, {
...
export function formatPresetData(
	...
	if (data.length >= 7 && matchesBytes(data, [240, 68, 0, 0, data[4], 33])) {
...
export async function restorePresets(
	...
	for (const filePath of filePaths) {
		...
		if (fileExists) {
			...
			const formattedData = formatPresetData(
				new Uint8Array(data),
				channel,
				target,
			);
...
export async function restorePresetToBuffer(
	...
	output.sendSysex([], formattedData.slice(1, -1));
	console.log(
		`Preset from ${preset.filename} restored to buffer successfully.`,
	);
	await new Promise((resolve) => setTimeout(resolve, 100));
}
