...
describe('filterPresets', () => {
  ...
  it('filters by search term (case-insensitive)', () => {
    ...
    expect(result.every((p) => p.name.toLowerCase().includes('bass'))).toBe(true)
  })
  ...
  it('detects duplicates via sysexData fingerprint', () => {
    ...
    const unique = makePreset({ name: 'Unique', sysexData: new Uint8Array([0xf0, 0x99, 0xf7]) })
  ...
  it('shuffles deterministically with the same seed', () => {
    const seed = 42
    const { presets: run1 } = filterPresets(presets, { ...baseOptions, randomOrder: true, seed })
    const { presets: run2 } = filterPresets(presets, { ...baseOptions, randomOrder: true, seed })
  ...
  it('different seeds produce different orderings for large sets', () => {
    ...
    const { presets: a } = filterPresets(manyPresets, { ...baseOptions, randomOrder: true, seed: 1 })
    const { presets: b } = filterPresets(manyPresets, { ...baseOptions, randomOrder: true, seed: 99 })
