...
describe("formatPresetData", () => {
	...
	it("normalizes 0x21 voice dump responses regardless of slot byte", () => {
		const responseSlot1 = new Uint8Array([
			0xf0, 0x44, 0x00, 0x00, 0x70, 0x21, 0x20, 0x01, 0x02, 0xf7,
		]);
		const responseSlot16 = new Uint8Array([
			0xf0, 0x44, 0x00, 0x00, 0x70, 0x21, 0x2f, 0x01, 0x02, 0xf7,
		]);
...
describe("createPresetData", () => {
	...
	it("should preserve the original packet bytes when creating a standard preset", async () => {
		const sysexData = new Uint8Array([
			0xf0, 0x44, 0x00, 0x00, 0x70, 0x20, 0x24, 0x0a, 0x00, 0x06, 0x02, 0x0d,
			0xf7,
		]);
	...
	it("should convert backup packets to standard preset headers", async () => {
		const sysexData = new Uint8Array([
			0xf0, 0x44, 0x00, 0x00, 0x70, 0x30, 0x0a, 0x00, 0x06, 0x02, 0x0d, 0xf7,
		]);
		...
		expect(Array.from(presets[0].sysexData)).toEqual([
			0xf0, 0x44, 0x00, 0x00, 0x70, 0x20, 0x21, 0x0a, 0x00, 0x06, 0x02, 0x0d,
			0xf7,
		]);
	});
	...
	it("should parse multiple presets from SysEx data", async () => {
		const sysexData = new Uint8Array([
			0xf0, 0x68, 0x00, 0x00, 0x70, 0x20, 0x60, 0x00, 0xf7, 0xf0, 0x68, 0x00,
			0x00, 0x70, 0x20, 0x61, 0x00, 0xf7,
		]);
	...
	it("should handle malformed SysEx data without F7 terminator", async () => {
		const sysexData = new Uint8Array([
			0xf0, 0x68, 0x00, 0x00, 0x70, 0x20, 0x60,
		]);
...
describe("fetchPresetData", () => {
	...
	it("should search presets by name", async () => {
		...
		expect(
			result.presets.filter((p) => p.name.toLowerCase().includes("bass")),
		).toHaveLength(1);
	...
	it("should filter presets by tags (exclusive)", async () => {
		...
		expect(result.presets.filter((p) => p.tags.includes("pad"))).toHaveLength(
			1,
		);
	});
	...
	it("should filter to duplicates only when requested", async () => {
		const uniquePreset: Preset = {
			...
			sysexData: new Uint8Array([
				0xf0, 0x44, 0x00, 0x00, 0x70, 0x20, 0x01, 0xf7,
			]),
			tags: ["lead"],
			author: "Test",
			description: "Unique",
	...
	it("should handle pagination offset", async () => {
		const firstPage = await fetchPresetData(
			0,
			2,
			[],
			"",
		...
		const secondPage = await fetchPresetData(
			2,
			2,
			[],
			"",
