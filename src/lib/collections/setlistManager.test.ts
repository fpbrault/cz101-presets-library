...
describe("setlistManager", () => {
	...
	describe("createSetlist", () => {
		...
		it("persists sysexData as Uint8Array", () => {
			...
			expect(Array.from(setlist.entries[0].sysexData)).toEqual([
				0xf0, 0x01, 0x02, 0xf7,
			]);
		});
	...
	describe("getSetlists", () => {
		...
		it("returns setlists sorted by createdAt descending", async () => {
			const first = createSetlist({
				name: "First",
				source: "manual",
				entries: [],
			});
			await new Promise((resolve) => setTimeout(resolve, 2));
			const second = createSetlist({
				name: "Second",
				source: "manual",
				entries: [],
			});
		...
		it("deserializes sysexData back to Uint8Array", () => {
			createSetlist({
				name: "X",
				source: "manual",
				entries: [makeEntry(1, [0xf0, 0xaa, 0xf7])],
			});
			const all = getSetlists();
	...
	describe("exportSetlist / importSetlist", () => {
		...
		it("imports setlist with sysexData intact as Uint8Array", () => {
			...
			expect(Array.from(imported.entries[0].sysexData)).toEqual([
				0xf0, 0xbb, 0xcc, 0xf7,
			]);
		});
	});
