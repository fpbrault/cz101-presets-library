...
function makeZeroPacket(): Uint8Array {
  return new Uint8Array([
    0xf0, 0x44, 0x00, 0x00, 0x70, 0x20, 0x60, // 7-byte header
...
function setLogicalByte(packet: Uint8Array, logIdx: number, value: number): void {
...
describe('decodeCzPatch', () => {
  ...
  it('returns null when payload contains non-nibble bytes', () => {
    const bad = new Uint8Array([
      0xf0, 0x44, 0x00, 0x00, 0x70, 0x20, 0x60,
  ...
  it('returns envelope objects with 8 steps each', () => {
    const packet = makeZeroPacket()
    const result = decodeCzPatch(packet)!
    for (const env of [result.dca1, result.dcw1, result.dco1Env, result.dca2, result.dcw2, result.dco2Env]) {
