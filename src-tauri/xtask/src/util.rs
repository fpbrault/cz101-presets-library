...
pub fn detect_has_gui(package: &str, workspace_root: &Path) -> bool {
    let pkg_dir = workspace_root.join("src-tauri").join(package);
...
///
/// Tries to read `src-tauri/{package}/Config.toml` first. Falls back to
/// parsing the source code in `src-tauri/{package}/src/lib.rs` if the TOML
