...
class CzSynthWorkletProcessor extends AudioWorkletProcessor {
	...
	_initWasm(wasmBytes, bindingsJs) {
		try {
			...
			// strip the leading `let/const <name> =` to turn the statement into
			// an expression that eval() returns, then assign it to globalThis.
			...
			globalThis.wasm_bindgen = globalThis.eval(
				bindingsJs.replace(/^\s*(?:let|const)\s+\w+\s*=/, ""),
			);
